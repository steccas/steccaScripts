...
for input_image in "$INPUT_DIR"/*; do
  if [[ -f "$input_image" ]]; then
    ...
    # Use ImageMagick to remove white background and convert to WebP
    magick "$input_image" -fuzz 10% -transparent white "$output_image"

    if [ $? -eq 0 ]; then
      echo "Image background removed and converted to WebP successfully: $output_image"
    else
